...
def r_t_cat(row):
    if((row["R_t"] >= 0.45) & (row["R_t"] <= 0.55)):
        return 0.5
    elif((row["R_t"] >= 0.95) & (row["R_t"] <= 1.05)):
        return 1.0
    elif((row["R_t"] >= 1.45) & (row["R_t"] <= 1.55)):
        return 1.5
    else:
        return np.nan
...
def plot(ax,input_type,adapt_mode):
    ...
    try:
        ...
        file_search = glob.glob(os.path.join(DATA_DIR, input_type + '_input_ESN/performance_sweep/param_sweep_performance_R_t_' +
        adapt_mode + '_*'))
        ...
        sweep_df.to_hdf(os.path.join(DATA_DIR, input_type + '_input_ESN/performance_sweep/performance_sweep/param_sweep_performance_R_t_' +
        adapt_mode + '_processed_data.h5'),'table')
    ...
    sweep_df = sweep_df.loc[sweep_df["sigm_e"] <= 1.]
    sweep_df['r_t_cat'] = sweep_df.apply (lambda row: r_t_cat(row), axis=1)

    sweep_df["delta_specrad_R_t"] = sweep_df["specrad"] - sweep_df["R_t"]

    sns.lineplot(ax=ax,data=sweep_df,x="sigm_e",y="delta_specrad_R_t",hue="r_t_cat",legend="full",palette="viridis",hue_norm=(0.5,1.8))
