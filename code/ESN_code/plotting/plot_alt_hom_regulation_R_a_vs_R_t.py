...
def plot(ax,input_type,adapt_mode):
    ...
    '''
    sns.lineplot(ax=ax,data=sweep_df,x="r_a",y="specrad",hue="sigm_e",legend="full")

    h,l = ax.get_legend_handles_labels()
    n_h = len(h)
    ax.legend([h[0],h[int(n_h/2)],h[-1]],[0.,"{:.1f}".format(float(l[int(n_h/2)])),l[-1]],title="$\\sigma_{\\rm ext}$")
