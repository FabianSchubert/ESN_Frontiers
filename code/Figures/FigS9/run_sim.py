...
for k in tqdm(range(n_sweep_sigm_e)):
    for l in tqdm(range(n_sweep_R_t),leave=False):
        ...
        rnn.R_target = R_t[l]
        rnn.w_in = np.ones((rnn.N,1))*sigm_e[k]

        ###### adaptation
        u_in_adapt = (np.random.rand(T_adapt) <= 0.5) * 2. - 1.

        rnn.run_hom_adapt("variance",u_in=u_in_adapt,show_progress=False)
...
for k in tqdm(range(n_sweep_sigm_e)):
    for l in tqdm(range(n_sweep_R_t),leave=False):
        ...
        rnn.run_hom_adapt("variance",T=T_adapt,sigm_e=np.abs(rnn.w_in[:,0]),show_progress=False)
